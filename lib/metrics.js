...
function readCpuTimes() {
  try {
    ...
    console.error('[metrics] readCpuTimes failed:', e.message);
...
function memInfo() {
  try {
    ...
    console.error('[metrics] memInfo failed:', e.message);
    return {
...
function loadAvg() {
  ...
  const normalized = [
    Number((one / cores).toFixed(2)),
    Number((five / cores).toFixed(2)),
    Number((fifteen / cores).toFixed(2)),
  ];
  return { one, five, fifteen, cores, normalized };
}
...
/**
 * Resets the stored metric history (deletes all rows from metric_samples).
 * Safely invokes resetSamples() on the history database.
 */
